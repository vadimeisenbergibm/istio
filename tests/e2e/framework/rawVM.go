...
func (vm *GCPRawVM) provision() error {
	...
	if _, err := u.ShellMuteOutput(vm.baseCommand("describe")); err != nil {
		...
		// reset existing VM
		// TODO (chx) the GCP reset might not give us a clean slate as we assumed.
		// 			Pending investigation and test env preprovisioning, might be better off
		//			deleting and creating the VM.
		return err
	}
